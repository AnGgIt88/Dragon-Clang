...
This toolchain is built on Ubuntu 20.04 LTS, which uses glibc 2.27. Compatibility with older distributions cannot be guaranteed. Other libc implementations (such as musl) are not supported.
...
```bash
export PATH="$HOME/toolchains/Finix-Clang/bin:$PATH"
```
